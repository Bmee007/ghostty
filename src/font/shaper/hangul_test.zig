...
test "shape Hangul with a composed-only face" {
    ...
    const regular = try collection.add(alloc, try font.Face.init(
        lib,
        font.embedded.hangul_test,
        .{ .size = .{ .points = 12 } },
    ), .{ .style = .regular, .fallback = false, .size_adjustment = .none });
    try collection.completeStyles(alloc, .{});
    var grid = try font.SharedGrid.init(alloc, .{ .collection = collection });
    defer grid.deinit(alloc);
    var shaper = try font.Shaper.init(alloc, .{});
    defer shaper.deinit();

    // This bundled fixture always has syllables but no conjoining jamo.
    // System discovery would let this regression silently skip on some Macs.
        ...
        for ([_]?usize{ null, 3 }) |cursor_x| {
            for (cases) |case| {
                ...
                while (try it.next(alloc)) |run| {
                    ...
                    try testing.expectEqual(if (bold) font.Style.bold else font.Style.regular, run.font_index.style);
